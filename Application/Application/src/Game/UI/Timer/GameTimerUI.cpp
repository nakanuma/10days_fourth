...
void GameTimerUI::Initialize(Cygnus::SpriteCommon* spriteCommon) {
	...
	texCountdown_ = Cygnus::TextureManager::Load("countdown.png");
	...
	spriteCountdown_->Initialize(spriteCommon, texCountdown_);
	spriteCountdown_->SetAnchorPoint(kAnchorPointCenter);
	spriteCountdown_->SetPosition({ 640.0f, 360.0f }); // 画面中央に配置
	spriteCountdown_->SetSize(kCdTextureSize);
	spriteCountdown_->SetTextureSize(kCdTextureSize);
}
...
void GameTimerUI::Countdown(float time) {
	//ラムダ式（spriteのnを引数として）
	auto setCountdown = [&](int digit, float startTime) {

		// カウントダウン用スプライトのテクスチャ切り替え
		spriteCountdown_->SetTextureLeftTop({ digit * kCdTextureSize.x, 0.0f });

		// その数字が表示されてからの経過時間（0.0 ～ 1.0）
		float t = startTime - time;

		// 大 → 小
		float scale = 1.0f - Cygnus::Easing::EaseInSine(t);

		spriteCountdown_->SetSize(kCdTextureSize * scale);
		};

	//timeが5秒より大きい場合はカウントダウンを表示しない
	if (time > 5.0f) {
		spriteCountdown_->SetColor({ 1.0f, 1.0f, 1.0f, 0.0f }); // 完全に透明にする
		return;
	}
	else {
		spriteCountdown_->SetColor({ 1.0f, 1.0f, 1.0f, 1.0f }); // 完全に不透明にする
	}

	if (time <= 5.0f && time > 4.0f) {
		setCountdown(0, 5.0f);
	}
	else if (time <= 4.0f && time > 3.0f) {
		setCountdown(1, 4.0f);
	}
