...
void ExplainUI::Update(Player* player) {
	...
	ActionJudgment::GetInstance()->IsAction(autoReadTime_ >= kO2TimeExplainTime_, 3);//説明 自動で酸素がなくなる
	
	auto input = Cygnus::Input::GetInstance();
	ActionJudgment::GetInstance()->IsAction(input->TriggerKey(DIK_SPACE) || 
		input->IsTriggerButton(0, XINPUT_GAMEPAD_A) || input->IsTriggerButton(0, XINPUT_GAMEPAD_RIGHT_THUMB), 5);//説明 命綱を巻き、宇宙船に帰還する
	
	ActionJudgment::GetInstance()->IsAction(autoReadTime_ >= kMeteorExplainTime_, 6);//説明 隕石注意
	ActionJudgment::GetInstance()->IsAction(autoReadTime_ >= kItemExplainTime_, 7);//説明 アイテム
	ActionJudgment::GetInstance()->IsAction(autoReadTime_ >= kPurposeExplainTime_, 8);//説明 ゲームの目的
