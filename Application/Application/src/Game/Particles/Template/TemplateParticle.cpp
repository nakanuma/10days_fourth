...
TemplateParticle::TemplateParticle(Cygnus::ModelManager::ModelData& model) {
	// オブジェクト設定
...
TemplateParticleData TemplateParticle::CreateParticle(const Cygnus::Float3& pos, const Cygnus::Float3& velocity, float angle) {
	TemplateParticleData p;
...
void TemplateParticle::UpdateParticle(TemplateParticleData& p, float dt) {
	...
	// 縮小
	float easeT = Cygnus::Easing::EaseInQuart(t);
