...
void BombItem::Initialize(const Cygnus::Float3& position, bool isRightToLeft) {
	FlyingObject::Initialize(position, isRightToLeft);

	// 各パラメーター設定
	category_ = ObjectCategory::BombItem;
	speed_ = kSpeed;
	...
	// モデル設定
	object_->model_ = &Cygnus::ModelManager::GetInstance()->GetModel("BombItem");

	// コライダー生成
	auto aabb = std::make_unique<Cygnus::AABBCollider>();
	aabb->SetTag("BombItem");
